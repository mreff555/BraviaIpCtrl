...
// to call the constructor at some point.  Not really sure the best way to
// implement this, so for now both stay
...
int main(int argc, const char *argv[]) {
  ...
  if (argc > 2 && argc < 5) {
    ...
    if (!strcmp(argv[2], "power") && !argv[3]) {
      ...
      // TODO: This is not working as expected.  There appears to be an issue
      // with logging enumerations
      // TODO: Info logging is defaulted however this message is not displayed
      // with info (displayed with warning)
      FILE_LOG(logINFO) << "Message Data: " << message.datagram.data;
    }
    ...
    std::cout << std::setw(2) << ""
              << "power\n";
    std::cout << std::setw(4) << ""
              << "<no argument>" << std::setw(33) << "Power status message\n";
    std::cout << std::setw(4) << ""
              << "status" << std::setw(40) << "Power status message\n";
    std::cout << std::setw(4) << ""
              << "on" << std::setw(37) << "Turn power on\n";
    std::cout << std::setw(4) << ""
              << "off" << std::setw(37) << "Turn power off\n";
    std::cout << "\n";
    std::cout << std::setw(2) << ""
              << "volume\n";
    std::cout << std::setw(4) << ""
              << "<no argument>" << std::setw(34) << "Volume status message\n";
    std::cout << std::setw(4) << ""
              << "status" << std::setw(41) << "Volume status message\n";
    std::cout << std::setw(4) << ""
              << "<value>" << std::setw(42) << "Integral value, 0 - 100\n";
    std::cout << "\n";
    std::cout << "IIRC Commands (No values)\n";
    std::cout << std::setw(2) << ""
              << "display" << std::setw(42) << "Description here\n";
    std::cout << std::setw(2) << ""
              << "home" << std::setw(42) << "Return to the home menu\n";
    std::cout << std::setw(2) << ""
              << "up" << std::setw(42) << "up\n";
    std::cout << std::setw(2) << ""
              << "down" << std::setw(42) << "down\n";
    std::cout << std::setw(2) << ""
              << "right" << std::setw(42) << "right\n";
    std::cout << std::setw(2) << ""
              << "left" << std::setw(42) << "left\n";
    std::cout << std::setw(2) << ""
              << "confirm" << std::setw(42) << "confirm / ok\n";
    std::cout << std::setw(2) << ""
              << "hdmi1" << std::setw(42) << "Switch to HDMI 1\n";

    // TODO: The input values are going to need to be reworked.  They are
    // relying on enum values.  This is fine internally but does not make for a
    // intuitive API.  Some type of string conversion will be neccessary.
  }
